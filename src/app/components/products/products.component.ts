...
export class ProductsComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    this.data.get("seller/getProducts").subscribe(res => {
  ...
  addSales() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.data.post("seller/addProduct", result).subscribe(
          res => {
  ...
  replenish(element) {
    ...
    dialogRef2.afterClosed().subscribe(result => {
      if (result) {
        this.data.post("seller/updateProduct", result).subscribe(
          res => {
...
export class AddProductsDialog implements OnInit, OnChanges {
  ...
  ngOnInit() {
    this.ds.get("seller/getProductCategories").subscribe(res => {
      this.dataSource.data = this.dataSource.data.concat(res);
      this.table.renderRows();
    });
    this.dataSource.paginator = this.paginator;
